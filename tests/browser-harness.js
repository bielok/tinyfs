const { READ, WRITE, READ_WRITE, CREATE, EXCLUSIVE, TRUNCATE, APPEND, SET, CURRENT, END, TYPE_MASK, TYPE_DIR, TYPE_FILE } = window.tinyfs;
...
test("stat root directory", async function () {
    ...
    if ((buf.mode & TYPE_MASK) !== TYPE_DIR)
...
test("stat a regular file", async function () {
    let fd = await tfs.open("/statfile", CREATE | READ_WRITE);
    ...
    if ((buf.mode & TYPE_MASK) !== TYPE_FILE)
...
test("rmdir on regular file returns -1", async function () {
    let fd = await tfs.open("/rmfile", CREATE | READ_WRITE);
    tfs.close(fd);
...
test("open with CREATE succeeds", async function () {
    let fd = await tfs.open("/f", CREATE | READ_WRITE);
...
test("EXCLUSIVE fails on existing file", async function () {
    let fd1 = await tfs.open("/exclf", CREATE | READ_WRITE);
    tfs.close(fd1);
    let fd2 = await tfs.open("/exclf", CREATE | EXCLUSIVE | READ_WRITE);
    return ok(fd2, -1);
});

test("open dir with WRONLY returns -1", async function () {
    await tfs.mkdir("/odir");
    let fd = await tfs.open("/odir", WRITE);
    return ok(fd, -1);
});

test("open without CREATE on missing file returns -1", async function () {
    let fd = await tfs.open("/nope", READ_WRITE);
    return ok(fd, -1);
});
...
test("write and read back", async function () {
    let fd   = await tfs.open("/hello", CREATE | READ_WRITE);
...
test("read 0 bytes at EOF on fresh file", async function () {
    let fd  = await tfs.open("/empty", CREATE | READ_WRITE);
...
test("write across block boundary", async function () {
    let fd   = await tfs.open("/big", CREATE | READ_WRITE);
...
test("READ rejects writes", async function () {
    let fd = await tfs.open("/ro", CREATE | READ);
...
test("WRITE rejects reads", async function () {
    let fd = await tfs.open("/wo", CREATE | WRITE);
...
test("write 0 bytes returns 0", async function () {
    let fd = await tfs.open("/zero", CREATE | READ_WRITE);
...
test("lseek SET CURRENT END", async function () {
    let fd = await tfs.open("/seek", CREATE | READ_WRITE);

    await tfs.write(fd, new Uint8Array(100).fill(0xFF), 100);

    let off1 = await tfs.lseek(fd, 10, SET);
    if (off1 !== 10)
        return "SET returned " + off1;

    let off2 = await tfs.lseek(fd, 5, CURRENT);
    if (off2 !== 15)
        return "CURRENT returned " + off2;

    let off3 = await tfs.lseek(fd, 0, END);
    if (off3 !== 100)
...
test("lseek invalid whence returns -1", async function () {
    let fd  = await tfs.open("/lseekbad", CREATE | READ_WRITE);
...
test("hard link preserves data", async function () {
    let fd   = await tfs.open("/a", CREATE | READ_WRITE);
    ...
    let fd2 = await tfs.open("/b", READ_WRITE);
...
test("TRUNCATE clears file", async function () {
    let fd = await tfs.open("/truncf", CREATE | READ_WRITE);

    await tfs.write(fd, new Uint8Array(100).fill(0xAA), 100);

    tfs.close(fd);

    let fd2 = await tfs.open("/truncf", TRUNCATE | READ_WRITE);
    let buf = { size: 0, mode: 0, nlink: 0 };
...
test("APPEND appends regardless of seek", async function () {
    let fd   = await tfs.open("/append", CREATE | READ_WRITE | APPEND);
...
test("readdir on regular file returns -1", async function () {
    let fd = await tfs.open("/rdirfile", CREATE | READ_WRITE);
    tfs.close(fd);
...
test("rename preserves data", async function () {
    let fd   = await tfs.open("/old", CREATE | READ_WRITE);
    ...
    let fd2 = await tfs.open("/new", READ_WRITE);
    let buf = new Uint8Array(3);
...
test("exhaust fd table gracefully", async function () {
    ...
    for (let i = 0; i < tfs.max_fd; i++)
    {
        let fd = await tfs.open("/fde_" + i, CREATE | READ_WRITE);
    ...
    let extra = await tfs.open("/fde_extra", CREATE | READ_WRITE);
    if (extra !== -1)
...
test("handle partial read", async function () {
    let fd   = await tfs.open("/partial", CREATE | READ_WRITE);
...
test("overwrite at offset after seek", async function () {
    let fd = await tfs.open("/rw", CREATE | READ_WRITE);
...
test("write on directory fd returns -1", async function () {
    ...
    let fd = await tfs.open("/writedir", READ);
    let nw = await tfs.write(fd, new Uint8Array([1]), 1);
...
test("read 0 bytes returns 0", async function () {
    let fd  = await tfs.open("/r0", CREATE | READ_WRITE);
...
test("read on directory fd returns -1", async function () {
    ...
    let fd = await tfs.open("/readdirfd", READ);
    let nr = await tfs.read(fd, new Uint8Array(1), 1);
...
test("lseek invalid fd returns -1", async function () {
    let ret = await tfs.lseek(-1, 0, SET);
    return ok(ret, -1);
});
...
test("decrement nlink on unlink", async function () {
    let fd = await tfs.open("/nl_a", CREATE | READ_WRITE);
    tfs.close(fd);
...
test("link target exists returns -1", async function () {
    let fd_a = await tfs.open("/tex_a", CREATE | READ_WRITE);
    tfs.close(fd_a);

    let fd_b = await tfs.open("/tex_b", CREATE | READ_WRITE);
    tfs.close(fd_b);
...
test("TRUNCATE no-op on empty file", async function () {
    let fd1 = await tfs.open("/trunc0", CREATE | READ_WRITE);

    tfs.close(fd1);

    let fd2 = await tfs.open("/trunc0", TRUNCATE | READ_WRITE);
    let buf = { size: 0, mode: 0, nlink: 0 };
...
test("TRUNCATE fails without CREATE on nonexistent file", async function () {
    let fd = await tfs.open("/nonexistent_trunc", TRUNCATE | READ_WRITE);
    return ok(fd, -1);
});
...
test("two fds to same file write via one read via the other", async function () {
    let fd1  = await tfs.open("/shared", CREATE | READ_WRITE);
    let fd2  = await tfs.open("/shared", READ_WRITE);
...
test("open unlink close close succeeds after unlink", async function () {
    let fd = await tfs.open("/unlinkme", CREATE | READ_WRITE);
...
test("write exactly BLOCK_SIZE bytes", async function () {
    let fd   = await tfs.open("/exactb", CREATE | READ_WRITE);
...
test("write BLOCK_SIZE + 1 bytes cross one boundary", async function () {
    let fd   = await tfs.open("/crossb", CREATE | READ_WRITE);
...
test("write 2 times BLOCK_SIZE bytes", async function () {
    let fd   = await tfs.open("/twofull", CREATE | READ_WRITE);
...
test("unlink one link preserves data via the other", async function () {
    let fd   = await tfs.open("/orig", CREATE | READ_WRITE);
    let data = new Uint8Array([10, 20, 30]);
    ...
    let fd2 = await tfs.open("/link", READ_WRITE);
    let buf = new Uint8Array(3);
    let nr  = await tfs.read(fd2, buf, 3);
...
test("unlink all links deletes file", async function () {
    let fd = await tfs.open("/hapath", CREATE | READ_WRITE);
    tfs.close(fd);
...
test("sparse file seek past EOF write read back", async function () {
    let fd       = await tfs.open("/sparse", CREATE | READ_WRITE);
    let gap      = 1000;
...
test("APPEND with two fds both append to end", async function () {
    let fd_a = await tfs.open("/dualapp", CREATE | READ_WRITE | APPEND);
    let fd_b = await tfs.open("/dualapp", READ_WRITE | APPEND);
...
test("fd slot reuse after close", async function () {
    ...
    for (let i = 0; i < tfs.max_fd; i++)
    {
        let fd = await tfs.open("/reuse_" + i, CREATE | READ_WRITE);
    ...
    let new_fd = await tfs.open("/reuse_new", CREATE | READ_WRITE);
    if (new_fd !== fds[0])
...
test("read fewer bytes than requested near EOF", async function () {
    let fd   = await tfs.open("/small", CREATE | READ_WRITE);
...
test("rename overwrite existing target", async function () {
    let fd_a = await tfs.open("/ren_a", CREATE | READ_WRITE);
    ...
    let fd_b = await tfs.open("/ren_b", CREATE | READ_WRITE);
    ...
    let fd  = await tfs.open("/ren_b", READ_WRITE);
    let buf = new Uint8Array(3);
...
test("rename target parent missing returns -1", async function () {
    let fd = await tfs.open("/rename_me", CREATE | READ_WRITE);
    tfs.close(fd);
